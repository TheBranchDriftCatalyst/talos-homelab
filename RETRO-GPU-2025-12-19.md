...
**Status**: ✅ RESOLVED - Cold boot fixed the GPU
...
| **Dec 19, 23:13** | Cold boot (full power cycle) performed |
| **Dec 19, 23:15** | ✅ **GPU WORKING** - QSV encoders verified functional |
...
**Root Cause**: GPU firmware entered a corrupted state during warm reboots on Dec 18.
Warm reboots preserve certain hardware states, including GPU firmware registers.

**Fix**: Cold boot (full power cycle - unplug power, wait, plug back in) reset the GPU
firmware to a clean state.

**Lesson Learned**: When GPU shows "wedged" errors, try a cold boot before driver changes.

## Action Items

- [x] Try rollback to Talos 1.11.1 + i915 - **DONE, STILL BROKEN** (GPU wedged)
- [x] **Cold boot** - ✅ **THIS FIXED IT**
- [ ] Document cold boot as standard GPU troubleshooting step
- [ ] Consider: xe driver may still work if cold boot was performed first
