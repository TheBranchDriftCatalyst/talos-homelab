...
EXPECTED FINDING: `frontmatter-schema` (error), "unterminated frontmatter block". The opening
`---` above is never closed, so there is no frontmatter and no body split.
