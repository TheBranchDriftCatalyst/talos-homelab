...
func ruleTaxonomyStructure(ctx *Ctx) []Finding {
	...
	for _, d := range ctx.Docs {
		...
		for _, heading := range ctx.Cfg.TypeRequires[dtype] {
			if !strings.Contains(strings.ToLower(d.Text), strings.ToLower(heading)) {
		...
		if ctx.Cfg.RequiredFooter != "" && dtype != "nav" && !strings.Contains(d.Text, ctx.Cfg.RequiredFooter) {
