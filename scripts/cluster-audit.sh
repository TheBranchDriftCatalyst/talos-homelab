...
RUNNING_PODS=$(kubectl --kubeconfig "$KUBECONFIG" get pods -A --no-headers 2>/dev/null | grep Running | wc -l | tr -d ' ')
PENDING_PODS=$(kubectl --kubeconfig "$KUBECONFIG" get pods -A --no-headers 2>/dev/null | grep Pending | wc -l | tr -d ' ')
FAILED_PODS=$(kubectl --kubeconfig "$KUBECONFIG" get pods -A --no-headers 2>/dev/null | grep -E '(Error|CrashLoop|Failed)' | wc -l | tr -d ' ')
...
TRAEFIK_CRDS=$(kubectl --kubeconfig "$KUBECONFIG" get crd --no-headers 2>/dev/null | grep traefik | wc -l | tr -d ' ')
