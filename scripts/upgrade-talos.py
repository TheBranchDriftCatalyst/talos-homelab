...
class UpgradeOrchestrator:
    ...
    def _warn_schematic_mismatches(self) -> None:
        ...
        """
        # NOT PORTED TO TALHELPER. Node extensions are declared inline in
        # configs/talconfig.yaml under schematic.customization.systemExtensions now, not as
        # standalone *-schematic.yaml files, so this check no longer has anything to read.
        # It is a warning-only cross-check, so losing it costs a warning, not correctness --
        # `task talos:verify-dry-run` compares the real installer URL against the generated
        # one and is the stronger check anyway.
        configs_dir = Path("configs/nodes")
        if not configs_dir.exists():
            log.info("schematic-mismatch check not ported to talhelper — extensions are "
                     "declared in configs/talconfig.yaml; use `task talos:verify-dry-run`")
            return
    ...
    def resolve_schematics_from_repo(self) -> None:
        ...
        if not any_resolved:
            log.info("no standalone schematic files (talhelper declares extensions inline in "
                     "configs/talconfig.yaml) — using each node's live installer URL")
    ...
    def bump_manifest_versions(self) -> None:
        """Bump `talosVersion:` in configs/talconfig.yaml to the version just installed.

        Was: rewrite `install.image:` in every configs/nodes/**/*.yaml. That tree is gone --
        machine configs are generated by talhelper now -- and the old code returned early on
        the missing directory, so after the cutover this quietly did nothing. The cluster
        would move to the new version while the repo still declared the old one, which is
        exactly the drift the talhelper migration existed to remove: `task talos:verify` would
        report a permanent installer mismatch, and a node rebuilt from the repo would come
        back on the version you had just upgraded away from.

        Under talhelper there is nothing per-node to rewrite. Installer URLs are derived from
        talosVersion plus each node's schematic (or talosImageURL), so one top-level line is
        the whole change.
        """
        talconfig = Path("configs/talconfig.yaml")

        if self.dry_run:
            log.info("[dry-run] would set talosVersion: %s in %s", self.target, talconfig)
            return

        if not talconfig.exists():
            # Loud, not a silent return: an unbumped talconfig.yaml is invisible until
            # someone rebuilds a node and it comes back on the old version.
            log.error("%s not found -- cannot record the upgrade in the repo", talconfig)
            log.error("the cluster is on %s but the repo still declares whatever it declared",
                      self.target)
            log.error("set `talosVersion: %s` by hand before anyone rebuilds a node",
                      self.target)
            return

        text = talconfig.read_text()
        # Anchored at column 0: talosVersion is top-level in talconfig.yaml, and an unanchored
        # match could hit a commented example or a nested key.
        new_text, n = re.subn(
            r"(?m)^(talosVersion:\s*)(\S+)\s*$",
            lambda m: f"{m.group(1)}{self.target}",
            text,
        )

        if n == 0:
            log.error("no top-level `talosVersion:` line in %s -- nothing bumped", talconfig)
            log.error("add `talosVersion: %s` by hand; the repo now disagrees with the cluster",
                      self.target)
            return

        if new_text == text:
            log.info("%s already declares talosVersion: %s", talconfig, self.target)
            return

        talconfig.write_text(new_text)
        log.info("%s: talosVersion -> %s", talconfig, self.target)
        log.info("review with: git diff configs/talconfig.yaml")
        log.info("then confirm the nodes agree: task talos:verify-dry-run")

        # Node schematics live inline in talconfig.yaml as extension lists now, and talhelper
        # resolves them against factory.talos.dev at genconfig time, so a version bump does not
        # require rewriting any schematic ID here.
    ...
    def run(self) -> bool:
        ...
        if self.update_manifests:
            ...
            log.warning("--no-update-manifests set; configs/talconfig.yaml still declares the "
                        "OLD talosVersion. Bump it by hand or a rebuilt node returns to it.")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--no-update-manifests", action="store_true",
                   help="don't bump talosVersion in configs/talconfig.yaml after success")
