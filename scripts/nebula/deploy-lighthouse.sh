...
echo "  nebula.knowledgedump.space → your home public IP"
echo ""
echo "AWS Secrets Manager (for EC2 workers):"
echo "  aws secretsmanager create-secret --name catalyst-llm/gpu-worker \\"
echo "    --secret-string '{"
echo "      \"nebula_ca_crt\": \"$(cat $CERT_DIR/ca.crt)\","
echo "      \"nebula_node_crt\": \"$(cat $CERT_DIR/gpu-worker-template.crt)\","
echo "      \"nebula_node_key\": \"$(cat $CERT_DIR/gpu-worker-template.key)\","
echo "      \"nebula_ip\": \"10.100.2.x\","
echo "      \"lighthouse_endpoint\": \"nebula.knowledgedump.space:4242\""
echo "    }'"
