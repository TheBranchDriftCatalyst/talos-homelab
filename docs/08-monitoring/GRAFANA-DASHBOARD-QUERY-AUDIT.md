...
Generated: 2026-08-19T23:31:41.448355+00:00
...
This section compares backend results with the live Kubernetes state observed during remediation. The detailed checklist below remains the evidence ledger.

| Check | Priority | Dashboard / scope | Finding | Evidence and action |
|---|---|---|---|---|
| [x] | P0 | Kasa dashboard query failures | Repaired all four backend-rejected PromQL expressions in the canonical `kasa-exporter` dashboard sources. | Live Mimir now accepts Active Alerts, Anomaly Count, Last Alert Time, and Battery State of Charge; the post-deployment audit has zero `FAIL` results. |
| [x] | P1 | KEDA Autoscaling — HPA panels | Removed the `keda-hpa-*` naming assumption and joined HPA scale targets to KEDA ScaledObjects. | Live validation finds both HPAs: Boomtime at 0/0 current/desired and Manyfold at 2/2, including min/max values. |
| [x] | P1 | Boomtime application dashboards | Fixed Grafana all-value auditing and enabled RabbitMQ per-object metrics so queue panels can select `boomtime.image-jobs`. | Post-rollout validation: 65 `PASS`, 39 healthy activity-dependent `EMPTY`, zero backend failures; all three dashboards render with HTTP 200 and zero query, console, page, or request errors. Reading-monitor advance panels remain empty because no advances occurred in the window, and failure/retry panels are healthy empty. |
| [x] | P2 | Active cluster-platform dashboards | Retired the Cilium 1.12 operator import, updated current MinIO, ExternalDNS, Cilium health, and LB-IPAM metric schemas, and made zero-valued failure panels explicit. | Pod Cleanup was a false empty caused by instant-only auditing of its two-hour remote-write series; range-aware validation now sees it. Local validation of the four changed dashboards is 151 `PASS`, with only healthy empty Pending/Lost PVC and filtered ExternalDNS activity panels. |
| [x] | P2 | Security and logging dashboards | Retired Grafana.com Loki dashboard 18042 because every query required a nonexistent `stream` label; retained the working Kubernetes Logs browser. | Cowrie and CrowdSec health/ingestion panels pass while attack, login, command, decision, and overflow panels are healthy event-dependent empties. Recreating the one stale `talos01` SPIRE agent cleared its outdated trust state and restored the DaemonSet to 5/5 ready. |
| [x] | P1 | CloudNativePG Databases | Confirmed as an auditor false positive, not missing telemetry. | Matcher-aware variable substitution now preserves embedded `$cluster` regexes; ten CNPG clusters are healthy and `cnpg_collector_up` exposes 25 instances. |
| [x] | P1 | Cilium Flows — Hubble Observer | Retired incompatible Grafana.com import 23862. | It requires a nonexistent `hubble-observer` log container; the supported Network Ops dashboard uses healthy native Hubble telemetry instead. |
| [x] | P1 | Ping Exporter / `flux-control-plane` | Found the real declarative source: Grafana.com ID 19761 resolves to unrelated Ping Exporter content. | Removed the misidentified `flux-control-plane` CR and backed up/deleted its stale Grafana dashboard. Flux Ops and Flux2 remain. |
| [x] | P1 | Dagster Pipelines | Expected idle state, not query drift. | All three Dagster Deployments are deliberately paused at `spec.replicas=0`; keep the dashboard and retest when resumed. |
| [ ] | P1 | KubeVirt Control Plane | Control plane is healthy, but 41 queries are empty because the imported dashboard uses legacy metrics/label assumptions. | Modernize this dashboard in the next batch against current `kubevirt_*` telemetry. |
| [x] | P2 | KubeVirt VM Info / VM Ops | Expected idle state. | The sole VM `gaming/windows-gameserver` is stopped and there are zero VMIs; retest workload panels when a VM runs. |
| [ ] | P2 | Empty-result corpus | 387 queries execute successfully but return no six-hour result. | Continue the per-dashboard review queue below; empty error/event panels may be healthy, while availability and inventory panels need live-state checks. |
| [ ] | P2 | Grafana template helpers | Seven Grafana `label_values` helpers require UI-side verification. | Backend APIs do not execute Grafana helper syntax directly; verify dropdown population during the next browser pass. |

Audit guarantees:

- All 49 remaining live `GrafanaDashboard` CRDs report `DashboardSynchronized=True` after reconciliation.
- Every one of the 1,491 remaining panel and query-variable expressions has a stable checklist row below.
- Every supported PromQL/LogQL expression was executed against live Mimir/Loki after resolving Grafana macros and variables.
- `PASS` means backend acceptance plus at least one result; it does not prove visualization semantics, thresholds, or units.
...
| [ ] | ArgoCD / Operational / Overview | 47 |
| [ ] | KubeVirt / Control Plane | 41 |
| [ ] | KubeVirt VM Ops | 31 |
| [ ] | KubeVirt VM Info | 27 |
| [ ] | ArgoCD / Application / Overview | 16 |
| [ ] | boomtime · Domain & Jobs | 16 |
| [ ] | boomtime · Reading monitor (whispersync cadence) | 13 |
| [ ] | 🚨 Alerts & Thresholds | 12 |
| [ ] | Pod Cleanup Job | 10 |
| [ ] | ArgoCD | 9 |
| [ ] | boomtime · Service & Infra | 9 |
| [ ] | Cowrie Ops | 9 |
| [ ] | Logging Dashboard via Loki v2 | 9 |
...
| [x] | monitoring | kasa-alerts-monitoring | https://raw.githubusercontent.com/TheBranchDriftCatalyst/kasa-exporter/main/etc/grafana/provisioning/dashboards/dashboards/6-alerts-monitoring.json?v=cf7fb16 | True |
| [x] | monitoring | kasa-battery-sizing | https://raw.githubusercontent.com/TheBranchDriftCatalyst/kasa-exporter/main/etc/grafana/provisioning/dashboards/dashboards/3-battery-sizing.json?v=cf7fb16 | True |
...
| [ ] | EMPTY | variable | Variable: namespace | namespace | ${datasource} | `label_values(argocd_app_info{cluster="$cluster"}, namespace)` | query executed successfully but returned no data |
| [ ] | REVIEW | variable | Variable: job | job | ${datasource} | `label_values(job)` | Grafana variable helper requires UI evaluation |
| [ ] | EMPTY | variable | Variable: kubernetes_cluster | kubernetes_cluster | ${datasource} | `label_values(argocd_app_info{cluster="$cluster", namespace=~"$namespace", job=~"$job"}, dest_server)` | query executed successfully but returned no data |
| [ ] | EMPTY | variable | Variable: project | project | ${datasource} | `label_values(argocd_app_info{cluster="$cluster", namespace=~"$namespace", job=~"$job", dest_server=~"$kubernetes_cluster"}, project)` | query executed successfully but returned no data |
| [ ] | EMPTY | variable | Variable: application_namespace | application_namespace | ${datasource} | `label_values(argocd_app_info{cluster="$cluster", namespace=~"$namespace", job=~"$job", dest_server=~"$kubernetes_cluster", project=~"$project"}, exported_namespace)` | query executed successfully but returned no data |
| [ ] | EMPTY | variable | Variable: application | application | ${datasource} | `label_values(argocd_app_info{cluster="$cluster", namespace=~"$namespace", job=~"$job", dest_server=~"$kubernetes_cluster", project=~"$project", exported_namespace=~"$application_namespace"}, name)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Application Health Status |  | default | `sum(   argocd_app_info{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  , dest_server=~"$kubernetes_cluster", project=~"$project"    } ) by (job, project, health_status)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Application Sync Status |  | default | `sum(   argocd_app_info{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  , dest_server=~"$kubernetes_cluster", project=~"$project"    } ) by (job, project, sync_status)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Application Syncs |  | default | `sum(   round(     increase(       argocd_app_sync_total{         cluster="$cluster", namespace=~"$namespace", job=~"$job"  , dest_server=~"$kubernetes_cluster", project=~"$project"        }[$__rate_interval]     )   ) ) by (job, project, phase)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Application Auto Sync Enabled |  | default | `sum(   argocd_app_info{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  , dest_server=~"$kubernetes_cluster", project=~"$project"    } ) by (job, project, autosync_enabled)` | query executed successfully but returned no data |
...
| [ ] | EMPTY | variable | Variable: kubernetes_cluster | kubernetes_cluster | ${datasource} | `label_values(argocd_app_info{cluster="$cluster", namespace=~"$namespace", job=~"$job"}, dest_server)` | query executed successfully but returned no data |
| [ ] | EMPTY | variable | Variable: project | project | ${datasource} | `label_values(argocd_app_info{cluster="$cluster", namespace=~"$namespace", job=~"$job", dest_server=~"$kubernetes_cluster"}, project)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Clusters |  | default | `sum(   argocd_cluster_info{     cluster="$cluster", namespace=~"$namespace", job=~"$job"     } )` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Repositories |  | default | `count(   count(     argocd_app_info{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }   )   by (repo) )` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Applications |  | default | `sum(   argocd_app_info{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  , dest_server=~"$kubernetes_cluster", project=~"$project"    } )` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Health Status |  | default | `sum(   argocd_app_info{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  , dest_server=~"$kubernetes_cluster", project=~"$project"    } ) by (health_status)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Sync Status |  | default | `sum(   argocd_app_info{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  , dest_server=~"$kubernetes_cluster", project=~"$project"    } ) by (sync_status)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Applications |  | default | `sum(   argocd_app_info{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  , dest_server=~"$kubernetes_cluster", project=~"$project"    } ) by (job, dest_server, project, name, health_status, sync_status)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Sync Activity |  | default | `sum(   round(     increase(       argocd_app_sync_total{         cluster="$cluster", namespace=~"$namespace", job=~"$job"  , dest_server=~"$kubernetes_cluster", project=~"$project"        }[$__rate_interval]     )   ) ) by (job, dest_server, project, name) > 0` | query executed successfully but returned no data |
...
| [ ] | EMPTY | panel | Reconciliation Activity |  | default | `sum(   round(     increase(       argocd_app_reconcile_count{         cluster="$cluster", namespace=~"$namespace", job=~"$job"         }[$__rate_interval]     )   ) ) by (namespace, job)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Reconciliation Performance |  | default | `sum(   rate(     argocd_app_reconcile_bucket{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (le)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | K8s API Activity |  | default | `sum(   round(     increase(       argocd_app_k8s_request_total{         cluster="$cluster", namespace=~"$namespace", job=~"$job"         }[$__rate_interval]     )   ) ) by (job, verb, resource_kind)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Resource Event Processing Duration |  | default | `sum(   rate(     argocd_resource_events_processing_bucket{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (le)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Resource Events Batch Size |  | default | `sum(   argocd_resource_events_processed_in_batch{     cluster="$cluster", namespace=~"$namespace", job=~"$job"     } ) by (namespace, job)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Pending Kubectl Runs |  | default | `sum(   argocd_kubectl_exec_pending{     cluster="$cluster", namespace=~"$namespace", job=~"$job"     } ) by (job, command)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Exec Total |  | default | `sum(   round(     increase(       argocd_kubectl_exec_total{         cluster="$cluster", namespace=~"$namespace", job=~"$job"         }[$__rate_interval]     )   ) ) by (namespace, job, command)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Requests Total |  | default | `sum(   rate(     argocd_kubectl_requests_total{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (container, method, code)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Requests Total by Host (6h) |  | default | `topk(20,   sum(     increase(       argocd_kubectl_requests_total{         cluster="$cluster", namespace=~"$namespace", job=~"$job"         }[6h]     )   ) by (host) )` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Request Duration by Verb (P95) |  | default | `histogram_quantile(   0.95,   sum(     rate(       argocd_kubectl_request_duration_seconds_bucket{         cluster="$cluster", namespace=~"$namespace", job=~"$job"         }[$__rate_interval]     )   ) by (le, container, verb) )` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Request Duration by Host (6h) |  | default | `topk(20,   histogram_quantile(     0.95,     sum(       increase(         argocd_kubectl_request_duration_seconds_bucket{           cluster="$cluster", namespace=~"$namespace", job=~"$job"           }[6h]       )     ) by (le, host)   ) )` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Request Retries |  | default | `sum(   rate(     argocd_kubectl_request_retries_total{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (container, method, code)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Request Retries by Host (6h) |  | default | `topk(20,   sum(     increase(       argocd_kubectl_request_retries_total{         cluster="$cluster", namespace=~"$namespace", job=~"$job"         }[6h]     )   ) by (host) )` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Rate Limiter Duration (P95) |  | default | `histogram_quantile(   0.95,   sum(     rate(       argocd_kubectl_rate_limiter_duration_seconds_bucket{         cluster="$cluster", namespace=~"$namespace", job=~"$job"         }[$__rate_interval]     )   ) by (le, verb) )` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Rate Limiter Duration P95 by Host (6h) |  | default | `topk(20,   histogram_quantile(     0.95,     sum(       increase(         argocd_kubectl_rate_limiter_duration_seconds_bucket{           cluster="$cluster", namespace=~"$namespace", job=~"$job"           }[6h]       )     ) by (le, host)   ) )` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Request Size |  | default | `sum(   rate(     argocd_kubectl_request_size_bytes_bucket{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (le)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Kubectl Response Size |  | default | `sum(   rate(     argocd_kubectl_response_size_bytes_bucket{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (le)` | query executed successfully but returned no data |
...
| [ ] | EMPTY | panel | Kubectl Transport Create Calls |  | default | `sum(   round(     increase(       argocd_kubectl_transport_create_calls_total{         cluster="$cluster", namespace=~"$namespace", job=~"$job"         }[$__rate_interval]     )   ) ) by (namespace, job)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | gRPC Requests Handled |  | default | `sum(   rate(     grpc_server_handled_total{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (namespace, job, grpc_service, grpc_method, grpc_code) > 0` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | gRPC Messages Sent |  | default | `sum(   rate(     grpc_server_msg_sent_total{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (namespace, job, grpc_service, grpc_method) > 0` | query executed successfully but returned no data |
...
| [ ] | EMPTY | panel | Cluster Connection Status |  | default | `sum(   argocd_cluster_connection_status{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  ,     server=~"$kubernetes_cluster"   } ) by (namespace, job, server, k8s_version)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Cluster Cache Age |  | default | `argocd_cluster_cache_age_seconds{   cluster="$cluster", namespace=~"$namespace", job=~"$job"  ,   server=~"$kubernetes_cluster" }` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Resource Objects |  | default | `sum(   argocd_cluster_api_resource_objects{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  ,     server=~"$kubernetes_cluster"   } ) by (namespace, job, server)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | API Resources |  | default | `sum(   argocd_cluster_api_resources{     cluster="$cluster", namespace=~"$namespace", job=~"$job"  ,     server=~"$kubernetes_cluster"   } ) by (namespace, job, server)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Cluster Events |  | default | `sum(   increase(     argocd_cluster_events_total{       cluster="$cluster", namespace=~"$namespace", job=~"$job"  ,       server=~"$kubernetes_cluster"     }[$__rate_interval]   ) ) by (namespace, job, server)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Pending Repo Requests |  | default | `sum(   argocd_repo_pending_request_total{     cluster="$cluster", namespace=~"$namespace", job=~"$job"     } ) by (namespace, job)` | query executed successfully but returned no data |
...
| [ ] | EMPTY | panel | Git Requests (ls-remote) |  | default | `sum(   increase(     argocd_git_request_total{       cluster="$cluster", namespace=~"$namespace", job=~"$job"  ,       request_type="ls-remote"     }[$__rate_interval]   ) ) by (namespace, job, repo)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Git Requests (checkout) |  | default | `sum(   increase(     argocd_git_request_total{       cluster="$cluster", namespace=~"$namespace", job=~"$job"  ,       request_type="fetch"     }[$__rate_interval]   ) ) by (namespace, job, repo)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Git Fetch Performance |  | default | `sum(   rate(     argocd_git_request_duration_seconds_bucket{       cluster="$cluster", namespace=~"$namespace", job=~"$job"  ,       request_type="fetch"     }[$__rate_interval]   ) ) by (le)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Git Ls-remote Performance |  | default | `sum(   rate(     argocd_git_request_duration_seconds_bucket{       cluster="$cluster", namespace=~"$namespace", job=~"$job"  ,       request_type="ls-remote"     }[$__rate_interval]   ) ) by (le)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Redis Request Rate |  | default | `sum(   rate(     argocd_redis_request_total{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (namespace, job, initiator)` | query executed successfully but returned no data |
| [ ] | EMPTY | panel | Redis Request Duration |  | default | `sum(   rate(     argocd_redis_request_duration_seconds_bucket{       cluster="$cluster", namespace=~"$namespace", job=~"$job"       }[$__rate_interval]   ) ) by (le)` | query executed successfully but returned no data |
...
| [x] | PASS | panel | External call rates — Hardcover & Amazon | B | mimir | `sum by (transport) (rate(amazon_calls_total{namespace="boomtime"}[$__rate_interval]))` | query executed and returned data |
...
| [x] | PASS | panel | Job runs by kind | A | loki-v2 | ``sum by (kind) (count_over_time({namespace="boomtime", app_kubernetes_io_name=~"$proc"} \|~ "jobs: done\|jobs: failed" \| json \| msg =~ `jobs: (done\|failed.*)` \| kind =~ "$kind" [$bucket]))`` | query executed and returned data |
| [x] | PASS | panel | Job kinds over the range | A | loki-v2 | `sum by (kind) (count_over_time({namespace="boomtime", app_kubernetes_io_name=~"$proc"} \|= "jobs: done" \| json \| msg = "jobs: done" \| kind =~ "$kind" [$__range]))` | query executed and returned data |
...
| [x] | PASS | panel | Job outcomes over time | A | loki-v2 | `sum(count_over_time({namespace="boomtime", app_kubernetes_io_name=~"$proc"} \|= "jobs: done" \| json \| msg = "jobs: done" \| kind =~ "$kind" [$bucket]))` | query executed and returned data |
...
| [x] | PASS | panel | Audible library items backfilled | A | loki-v2 | `sum(sum_over_time({namespace="boomtime", app_kubernetes_io_name=~"$proc"} \|= "audible backfill complete" \| json \| unwrap libraryItems [$__range]))` | query executed and returned data |
...
| [x] | PASS | panel | Read-path errors by status | A | loki-v2 | ``sum by (status) (count_over_time({namespace="boomtime", app_kubernetes_io_name=~"$proc"} \|~ "/api/" \| json \| msg = "http request" \| method = "GET" \| path =~ `/api/.+` \| status >= 400 [$bucket]))`` | query executed and returned data |
...
| [x] | PASS | panel | Warnings and errors (live) | A | loki-v2 | ``{namespace="boomtime", app_kubernetes_io_name=~"$proc"} \|~ `"level":"(WARN\|ERROR)"` \| json \| line_format "{{.level}}  {{.msg}}  {{.err}}"`` | query executed and returned data |
| [x] | PASS | panel | Ingest and aggregation failures | A | loki-v2 | `{namespace="boomtime", app_kubernetes_io_name=~"$proc"} \|~ "failed to store heartbeats\|aggregation query failed\|derived status failed\|failed to store" \| json \| line_format "{{.level}}  {{.msg}}  {{.err}}"` | query executed and returned data |
...
| [x] | PASS | panel | 5xx error ratio | A | mimir | `sum(rate(http_requests_total{namespace="boomtime",status_class="5xx"}[$__rate_interval])) / clamp_min(sum(rate(http_requests_total{namespace="boomtime"}[$__rate_interval])), 1e-9)` | query executed and returned data |
...
| [x] | PASS | panel | Error ratio by route (4xx+5xx) | A | mimir | `sum by (route) (rate(http_requests_total{namespace="boomtime",status_class=~"4xx\|5xx"}[$__rate_interval])) / clamp_min(sum by (route) (rate(http_requests_total{namespace="boomtime"}[$__rate_interval])), 1e-9)` | query executed and returned data |
...
| [x] | PASS | panel | Outbound error ratio by host | A | mimir | `sum by (host) (rate(http_client_requests_total{namespace="boomtime",status_class=~"4xx\|5xx\|error"}[$__rate_interval])) / clamp_min(sum by (host) (rate(http_client_requests_total{namespace="boomtime"}[$__rate_interval])), 1e-9)` | query executed and returned data |
...
| [x] | PASS | panel | Clusters | A | mimir | `count(max by (namespace, pod) (cnpg_pg_replication_in_recovery{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}) == 0)` | query executed and returned data |
| [x] | PASS | panel | Instances | A | mimir | `count(max by (namespace, pod) (cnpg_collector_up{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}))` | query executed and returned data |
| [x] | PASS | panel | Instances Healthy | A | mimir | `count(max by (namespace, pod) (cnpg_collector_up{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}) == 1)` | query executed and returned data |
| [x] | PASS | panel | Total DB Size | A | mimir | `sum(max by (namespace, datname) (cnpg_pg_database_size_bytes{datname!~"template.*", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}))` | query executed and returned data |
| [x] | PASS | panel | Connections | A | mimir | `sum(max by (namespace, pod, state) (cnpg_backends_total{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}))` | query executed and returned data |
| [x] | PASS | panel | Fleet TPS | A | mimir | `sum(max by (namespace, pod) (rate(cnpg_pg_stat_database_xact_commit{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[$__rate_interval]))) + sum(max by (namespace, pod) (rate(cnpg_pg_stat_database_xact_rollback{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[$__rate_interval])))` | query executed and returned data |
| [x] | PASS | panel | Provisioned Databases & Instances | Version | mimir | `max by (namespace, pod) (cnpg_collector_postgres_version{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Provisioned Databases & Instances | Role | mimir | `max by (namespace, pod) (cnpg_pg_replication_in_recovery{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Provisioned Databases & Instances | Started | mimir | `max by (namespace, pod) (cnpg_pg_postmaster_start_time{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}) * 1000` | query executed and returned data |
| [x] | PASS | panel | Provisioned Databases & Instances | Up | mimir | `max by (namespace, pod) (cnpg_collector_up{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Provisioned Databases & Instances | Conn | mimir | `sum by (namespace, pod) (max by (namespace, pod, state) (cnpg_backends_total{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}))` | query executed and returned data |
| [x] | PASS | panel | Provisioned Databases & Instances | Max | mimir | `max by (namespace, pod) (cnpg_pg_settings_setting{name="max_connections", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | CPU — requests / limits / usage | A | mimir | `sum by (namespace, pod) (rate(container_cpu_usage_seconds_total{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$", container="postgres"}[$__rate_interval]))` | query executed and returned data |
| [x] | PASS | panel | CPU — requests / limits / usage | B | mimir | `max by (namespace, pod) (kube_pod_container_resource_requests{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$", container="postgres", resource="cpu"})` | query executed and returned data |
| [x] | PASS | panel | CPU — requests / limits / usage | C | mimir | `max by (namespace, pod) (kube_pod_container_resource_limits{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$", container="postgres", resource="cpu"})` | query executed and returned data |
| [x] | PASS | panel | Memory — requests / limits / usage | A | mimir | `max by (namespace, pod) (container_memory_working_set_bytes{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$", container="postgres"})` | query executed and returned data |
| [x] | PASS | panel | Memory — requests / limits / usage | B | mimir | `max by (namespace, pod) (kube_pod_container_resource_requests{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$", container="postgres", resource="memory"})` | query executed and returned data |
| [x] | PASS | panel | Memory — requests / limits / usage | C | mimir | `max by (namespace, pod) (kube_pod_container_resource_limits{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$", container="postgres", resource="memory"})` | query executed and returned data |
| [x] | PASS | panel | Storage — provisioned PVC vs data size | A | mimir | `max by (namespace, persistentvolumeclaim) (kube_persistentvolumeclaim_resource_requests_storage_bytes{namespace=~"$namespace", persistentvolumeclaim=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Storage — provisioned PVC vs data size | B | mimir | `sum by (namespace, pod) (max by (namespace, pod, datname) (cnpg_pg_database_size_bytes{datname!~"template.*", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}))` | query executed and returned data |
| [x] | PASS | panel | Connections by state | A | mimir | `max by (namespace, pod, state) (cnpg_backends_total{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Connection usage % of max | A | mimir | `100 * sum by (namespace, pod) (max by (namespace, pod, state) (cnpg_backends_total{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})) / max by (namespace, pod) (cnpg_pg_settings_setting{name="max_connections", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Transactions/sec — commit vs rollback | A | mimir | `max by (namespace, pod) (rate(cnpg_pg_stat_database_xact_commit{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m]))` | query executed and returned data |
| [x] | PASS | panel | Transactions/sec — commit vs rollback | B | mimir | `max by (namespace, pod) (rate(cnpg_pg_stat_database_xact_rollback{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m]))` | query executed and returned data |
| [x] | PASS | panel | Cache hit ratio | A | mimir | `100 * max by (namespace, pod) (rate(cnpg_pg_stat_database_blks_hit{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])) / (max by (namespace, pod) (rate(cnpg_pg_stat_database_blks_hit{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])) + max by (namespace, pod) (rate(cnpg_pg_stat_database_blks_read{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])))` | query executed and returned data |
| [x] | PASS | panel | Tuple I/O [5m] | A | mimir | `sum(max by (namespace, pod) (rate(cnpg_pg_stat_database_tup_fetched{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])))` | query executed and returned data |
| [x] | PASS | panel | Tuple I/O [5m] | B | mimir | `sum(max by (namespace, pod) (rate(cnpg_pg_stat_database_tup_returned{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])))` | query executed and returned data |
| [x] | PASS | panel | Tuple I/O [5m] | C | mimir | `sum(max by (namespace, pod) (rate(cnpg_pg_stat_database_tup_inserted{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])))` | query executed and returned data |
| [x] | PASS | panel | Tuple I/O [5m] | D | mimir | `sum(max by (namespace, pod) (rate(cnpg_pg_stat_database_tup_updated{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])))` | query executed and returned data |
| [x] | PASS | panel | Tuple I/O [5m] | E | mimir | `sum(max by (namespace, pod) (rate(cnpg_pg_stat_database_tup_deleted{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])))` | query executed and returned data |
| [x] | PASS | panel | Deadlocks & temp bytes [5m] | A | mimir | `sum(max by (namespace, pod) (rate(cnpg_pg_stat_database_deadlocks{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])))` | query executed and returned data |
| [x] | PASS | panel | Deadlocks & temp bytes [5m] | B | mimir | `sum(max by (namespace, pod) (rate(cnpg_pg_stat_database_temp_bytes{datname="", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m])))` | query executed and returned data |
| [x] | PASS | panel | Replication lag | A | mimir | `max by (namespace, pod) (cnpg_pg_replication_lag{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Streaming replicas | A | mimir | `max by (namespace, pod) (cnpg_pg_replication_streaming_replicas{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | WAL segments | A | mimir | `max by (namespace, pod) (cnpg_collector_pg_wal{value="count", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | WAL archiver [5m] | A | mimir | `max by (namespace, pod) (rate(cnpg_pg_stat_archiver_archived_count{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m]))` | query executed and returned data |
| [x] | PASS | panel | WAL archiver [5m] | B | mimir | `max by (namespace, pod) (rate(cnpg_pg_stat_archiver_failed_count{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"}[5m]))` | query executed and returned data |
| [x] | PASS | panel | Time since last successful backup | A | mimir | `time() - max by (namespace, pod) ({__name__=~"cnpg_collector_last_available_backup_timestamp\|barman_cloud_cloudnative_pg_io_last_available_backup_timestamp", namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"} > 0)` | query executed and returned data |
| [x] | PASS | panel | Last collection error (0 = OK) | A | mimir | `max by (namespace, pod) (cnpg_collector_last_collection_error{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Metrics collection duration | A | mimir | `max by (namespace, pod) (cnpg_collector_collection_duration_seconds{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | XID / MXID wraparound age per database | XID | mimir | `max by (namespace, datname) (cnpg_pg_database_xid_age{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | XID / MXID wraparound age per database | MXID | mimir | `max by (namespace, datname) (cnpg_pg_database_mxid_age{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Max XID age (selected) | A | mimir | `max (cnpg_pg_database_xid_age{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Max MXID age (selected) | A | mimir | `max (cnpg_pg_database_mxid_age{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Time since last FAILED backup | A | mimir | `time() - max by (namespace, pod) (cnpg_collector_last_failed_backup_timestamp{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"} > 0)` | query executed and returned data |
| [x] | PASS | panel | PITR window (age of oldest recoverable point) | A | mimir | `time() - max by (namespace, pod) (cnpg_collector_first_recoverability_point{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"} > 0)` | query executed and returned data |
| [x] | PASS | panel | Seconds since last WAL archival | A | mimir | `max by (namespace, pod) (cnpg_pg_stat_archiver_seconds_since_last_archival{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Max transaction age (long-running txn) | A | mimir | `max by (namespace, pod) (cnpg_backends_max_tx_duration_seconds{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
| [x] | PASS | panel | Backends waiting on locks | A | mimir | `sum by (namespace, pod) (cnpg_backends_waiting_total{namespace=~"$namespace", pod=~"($cluster)-[0-9]+$"})` | query executed and returned data |
...
| [ ] | EMPTY | panel | external-dns Activity Log (heartbeat filtered) | A | loki-v2 | `` {namespace="external-dns", app_kubernetes_io_name="external-dns"} != `All records are already up to date` `` | query executed successfully but returned no data |
...
| [ ] | EMPTY | panel | Not-Ready Objects (now) | A | mimir | `gotk_reconcile_condition{type="Ready",status="False"} == 1` | query executed successfully but returned no data |
...
| [x] | PASS | panel | KEDA-managed HPAs | A | mimir | `count(max by (namespace, horizontalpodautoscaler, scaledObject) (kube_horizontalpodautoscaler_spec_max_replicas{namespace=~"$namespace"} * on (namespace, horizontalpodautoscaler) group_left (scaledObject) label_replace((max by (namespace, horizontalpodautoscaler, scaletargetref_name) (kube_horizontalpodautoscaler_info{namespace=~"$namespace"}) and on (namespace, scaletargetref_name) label_replace(label_replace(max by (exported_namespace, scaledObject) (keda_scaler_active{type="scaledobject", exported_namespace=~"$namespace", scaledObject=~"$scaledObject"}), "namespace", "$1", "exported_namespace", "(.+)"), "scaletargetref_name", "$1", "scaledObject", "(.+)")), "scaledObject", "$1", "scaletargetref_name", "(.+)")))` | query executed and returned data |
| [x] | PASS | panel | Current replicas (managed) | A | mimir | `sum(max by (namespace, horizontalpodautoscaler, scaledObject) (kube_horizontalpodautoscaler_status_current_replicas{namespace=~"$namespace"} * on (namespace, horizontalpodautoscaler) group_left (scaledObject) label_replace((max by (namespace, horizontalpodautoscaler, scaletargetref_name) (kube_horizontalpodautoscaler_info{namespace=~"$namespace"}) and on (namespace, scaletargetref_name) label_replace(label_replace(max by (exported_namespace, scaledObject) (keda_scaler_active{type="scaledobject", exported_namespace=~"$namespace", scaledObject=~"$scaledObject"}), "namespace", "$1", "exported_namespace", "(.+)"), "scaletargetref_name", "$1", "scaledObject", "(.+)")), "scaledObject", "$1", "scaletargetref_name", "(.+)")))` | query executed and returned data |
...
| [x] | PASS | panel | ScaledObjects — scaler activity & resulting HPA replicas | Cur | mimir | `max by (namespace, horizontalpodautoscaler, scaledObject) (kube_horizontalpodautoscaler_status_current_replicas{namespace=~"$namespace"} * on (namespace, horizontalpodautoscaler) group_left (scaledObject) label_replace((max by (namespace, horizontalpodautoscaler, scaletargetref_name) (kube_horizontalpodautoscaler_info{namespace=~"$namespace"}) and on (namespace, scaletargetref_name) label_replace(label_replace(max by (exported_namespace, scaledObject) (keda_scaler_active{type="scaledobject", exported_namespace=~"$namespace", scaledObject=~"$scaledObject"}), "namespace", "$1", "exported_namespace", "(.+)"), "scaletargetref_name", "$1", "scaledObject", "(.+)")), "scaledObject", "$1", "scaletargetref_name", "(.+)"))` | query executed and returned data |
| [x] | PASS | panel | ScaledObjects — scaler activity & resulting HPA replicas | Des | mimir | `max by (namespace, horizontalpodautoscaler, scaledObject) (kube_horizontalpodautoscaler_status_desired_replicas{namespace=~"$namespace"} * on (namespace, horizontalpodautoscaler) group_left (scaledObject) label_replace((max by (namespace, horizontalpodautoscaler, scaletargetref_name) (kube_horizontalpodautoscaler_info{namespace=~"$namespace"}) and on (namespace, scaletargetref_name) label_replace(label_replace(max by (exported_namespace, scaledObject) (keda_scaler_active{type="scaledobject", exported_namespace=~"$namespace", scaledObject=~"$scaledObject"}), "namespace", "$1", "exported_namespace", "(.+)"), "scaletargetref_name", "$1", "scaledObject", "(.+)")), "scaledObject", "$1", "scaletargetref_name", "(.+)"))` | query executed and returned data |
| [x] | PASS | panel | ScaledObjects — scaler activity & resulting HPA replicas | Min | mimir | `max by (namespace, horizontalpodautoscaler, scaledObject) (kube_horizontalpodautoscaler_spec_min_replicas{namespace=~"$namespace"} * on (namespace, horizontalpodautoscaler) group_left (scaledObject) label_replace((max by (namespace, horizontalpodautoscaler, scaletargetref_name) (kube_horizontalpodautoscaler_info{namespace=~"$namespace"}) and on (namespace, scaletargetref_name) label_replace(label_replace(max by (exported_namespace, scaledObject) (keda_scaler_active{type="scaledobject", exported_namespace=~"$namespace", scaledObject=~"$scaledObject"}), "namespace", "$1", "exported_namespace", "(.+)"), "scaletargetref_name", "$1", "scaledObject", "(.+)")), "scaledObject", "$1", "scaletargetref_name", "(.+)"))` | query executed and returned data |
| [x] | PASS | panel | ScaledObjects — scaler activity & resulting HPA replicas | Max | mimir | `max by (namespace, horizontalpodautoscaler, scaledObject) (kube_horizontalpodautoscaler_spec_max_replicas{namespace=~"$namespace"} * on (namespace, horizontalpodautoscaler) group_left (scaledObject) label_replace((max by (namespace, horizontalpodautoscaler, scaletargetref_name) (kube_horizontalpodautoscaler_info{namespace=~"$namespace"}) and on (namespace, scaletargetref_name) label_replace(label_replace(max by (exported_namespace, scaledObject) (keda_scaler_active{type="scaledobject", exported_namespace=~"$namespace", scaledObject=~"$scaledObject"}), "namespace", "$1", "exported_namespace", "(.+)"), "scaletargetref_name", "$1", "scaledObject", "(.+)")), "scaledObject", "$1", "scaletargetref_name", "(.+)"))` | query executed and returned data |
...
| [x] | PASS | panel | HPA replicas — current vs desired | A | mimir | `max by (namespace, horizontalpodautoscaler, scaledObject) (kube_horizontalpodautoscaler_status_current_replicas{namespace=~"$namespace"} * on (namespace, horizontalpodautoscaler) group_left (scaledObject) label_replace((max by (namespace, horizontalpodautoscaler, scaletargetref_name) (kube_horizontalpodautoscaler_info{namespace=~"$namespace"}) and on (namespace, scaletargetref_name) label_replace(label_replace(max by (exported_namespace, scaledObject) (keda_scaler_active{type="scaledobject", exported_namespace=~"$namespace", scaledObject=~"$scaledObject"}), "namespace", "$1", "exported_namespace", "(.+)"), "scaletargetref_name", "$1", "scaledObject", "(.+)")), "scaledObject", "$1", "scaletargetref_name", "(.+)"))` | query executed and returned data |
| [x] | PASS | panel | HPA replicas — current vs desired | B | mimir | `max by (namespace, horizontalpodautoscaler, scaledObject) (kube_horizontalpodautoscaler_status_desired_replicas{namespace=~"$namespace"} * on (namespace, horizontalpodautoscaler) group_left (scaledObject) label_replace((max by (namespace, horizontalpodautoscaler, scaletargetref_name) (kube_horizontalpodautoscaler_info{namespace=~"$namespace"}) and on (namespace, scaletargetref_name) label_replace(label_replace(max by (exported_namespace, scaledObject) (keda_scaler_active{type="scaledobject", exported_namespace=~"$namespace", scaledObject=~"$scaledObject"}), "namespace", "$1", "exported_namespace", "(.+)"), "scaletargetref_name", "$1", "scaledObject", "(.+)")), "scaledObject", "$1", "scaletargetref_name", "(.+)"))` | query executed and returned data |
...
| [ ] | EMPTY | panel | Total count of stderr / stdout pie | A | Loki | `sum(count_over_time(({container="$container", pod=~"$pod"})[$__interval])) by (stream)` | query executed successfully but returned no data |
...
| [ ] | EMPTY | panel | Count of stderr / stdout historical | A | Loki | `sum(count_over_time(({container="$container", pod=~"$pod"})[$__interval])) by (stream)` | query executed successfully but returned no data |
...
| [x] | PASS | panel | VPN Status |  | ${datasource} | `count(kube_pod_container_status_ready{namespace=~"vpn-gateway\|media", container="gluetun", pod=~"$service.*"} == 1)` | query executed and returned data |
...
| [x] | PASS | panel | VPN Tunnel Bandwidth by Service |  | ${datasource} | `sum by (service) (label_replace(rate(container_network_receive_bytes_total{namespace=~"vpn-gateway\|media", interface="tun0", pod=~"$service.*"}[$smoothing]), "service", "$1", "pod", "^([^-]+-?[^-]*)-[^-]+-[^-]+$"))` | query executed and returned data |
| [x] | PASS | panel | VPN Tunnel Bandwidth by Service |  | ${datasource} | `sum by (service) (label_replace(rate(container_network_transmit_bytes_total{namespace=~"vpn-gateway\|media", interface="tun0", pod=~"$service.*"}[$smoothing]), "service", "$1", "pod", "^([^-]+-?[^-]*)-[^-]+-[^-]+$"))` | query executed and returned data |
...
| [x] | PASS | panel | VPN Packet Rate |  | ${datasource} | `sum by (service) (label_replace(rate(container_network_receive_packets_total{namespace=~"vpn-gateway\|media", interface="tun0", pod=~"$service.*"}[$smoothing]), "service", "$1", "pod", "^([^-]+-?[^-]*)-[^-]+-[^-]+$"))` | query executed and returned data |
| [x] | PASS | panel | VPN Packet Rate |  | ${datasource} | `sum by (service) (label_replace(rate(container_network_transmit_packets_total{namespace=~"vpn-gateway\|media", interface="tun0", pod=~"$service.*"}[$smoothing]), "service", "$1", "pod", "^([^-]+-?[^-]*)-[^-]+-[^-]+$"))` | query executed and returned data |
...
| [x] | PASS | panel | VPN Tunnel Errors |  | ${datasource} | `sum by (service) (label_replace(rate(container_network_receive_errors_total{namespace=~"vpn-gateway\|media", interface="tun0", pod=~"$service.*"}[$smoothing]), "service", "$1", "pod", "^([^-]+-?[^-]*)-[^-]+-[^-]+$"))` | query executed and returned data |
| [x] | PASS | panel | VPN Tunnel Errors |  | ${datasource} | `sum by (service) (label_replace(rate(container_network_transmit_errors_total{namespace=~"vpn-gateway\|media", interface="tun0", pod=~"$service.*"}[$smoothing]), "service", "$1", "pod", "^([^-]+-?[^-]*)-[^-]+-[^-]+$"))` | query executed and returned data |
| [x] | PASS | panel | VPN Dropped Packets |  | ${datasource} | `sum by (service) (label_replace(rate(container_network_receive_packets_dropped_total{namespace=~"vpn-gateway\|media", interface="tun0", pod=~"$service.*"}[$smoothing]), "service", "$1", "pod", "^([^-]+-?[^-]*)-[^-]+-[^-]+$"))` | query executed and returned data |
| [x] | PASS | panel | VPN Dropped Packets |  | ${datasource} | `sum by (service) (label_replace(rate(container_network_transmit_packets_dropped_total{namespace=~"vpn-gateway\|media", interface="tun0", pod=~"$service.*"}[$smoothing]), "service", "$1", "pod", "^([^-]+-?[^-]*)-[^-]+-[^-]+$"))` | query executed and returned data |
...
| [ ] | EMPTY | panel | 🎯 Baseline Comparison (vs $baseline_device) | B | mimir | `current_consumption:by_device{version=~"$version", alias="$baseline_device"}` | query executed successfully but returned no data |
...
| [ ] | EMPTY | panel | 🎯 Baseline Comparison (vs $baseline_device) | D | mimir | `consumption_cost:by_device{version=~"$version", alias="$baseline_device"}` | query executed successfully but returned no data |
...
| [ ] | EMPTY | panel | 🎯 Baseline Comparison (vs $baseline_device) | F | mimir | `cost_efficiency:by_device{version=~"$version", alias="$baseline_device"}` | query executed successfully but returned no data |
...
| [x] | PASS | panel | 📈 Battery State of Charge Timeline | B | mimir | `clamp_max(vector((($solar_panel_watts * $inverter_efficiency) / ($battery_voltage * $battery_capacity_ah)) * 100 * (time() % 86400) / 3600), 100)` | query executed and returned data |
...
| [x] | PASS | panel | 🚨 Anomaly Count (Time Range) | A | prometheus | `sum(count_over_time((consumption_cost:anomaly == 1)[$__range:])) or vector(0)` | query executed and returned data |
...
| [x] | PASS | panel | 🚨 Active Alerts | A | prometheus | `(   count(current_consumption:total > $power_alert_threshold) or vector(0) ) + (   count(consumption_cost:total > $cost_alert_threshold) or vector(0) ) + (   count(consumption_cost:anomaly == 1) or vector(0) ) + (   count(rate_class:numeric == 3) or vector(0) ) + (   count(state{version=~"$version"} != 1) or vector(0) )` | query executed and returned data |
...
| [x] | PASS | panel | 🔔 Last Alert Time | A | prometheus | `time() - max(max_over_time(timestamp(current_consumption:total > $power_alert_threshold)[$__range:])) or vector(86400)` | query executed and returned data |
