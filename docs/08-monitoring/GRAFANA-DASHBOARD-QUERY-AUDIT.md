...
| [ ] | P1 | Dagster Pipelines | The CRD is synchronized, but all 61 queries are empty and the `catalyst-data` namespace has no pods. | This is a stale/orphan dashboard relative to cluster state. Remove it or redeploy Dagster and its telemetry. No `dagster_*` series exist in Mimir. |
...
| [ ] | P1 | Ping Exporter | All 11 queries are empty and no ping-exporter workload is deployed. | Remove the imported dashboard or deploy/scrape ping-exporter. |
| [ ] | P1 | Cilium Flows — Hubble Observer | All 9 queries are empty although Hubble relay/UI are running and the custom Network Ops dashboard returns data. | The imported dashboard is incompatible with the current labels/datasource. Retarget its variables/queries or retire it in favor of Network Ops. |
| [ ] | P1 | CloudNativePG Databases | 49 of 59 queries are empty while ten CNPG clusters are healthy. | Audit obsolete metric names and namespace/cluster filters; this is not explained by absent workloads. |
| [ ] | P1 | KEDA Autoscaling — HPA panels | HPA discovery assumes names match `keda-hpa-.*`, so the live Manyfold HPA named `manyfold-performance-worker` is omitted. | Discover KEDA-managed HPAs through labels/ownership rather than a name prefix. The visual audit showed an active Manyfold scaler but no Manyfold HPA row, while Kubernetes reports 2 current/desired replicas. |
