...
tickets:
  ...
  - TALOS-23l
  - TALOS-hg7
bluf: Four test layers all run under one pytest, selected by suite marker, and all are safe/offline by default — anything that touches the live cluster or destroys something is behind an explicit flag.
---
...
1. **DR / resilience layer** — per-component `test_<suite>_dr.py` suites (marker
...
2. **Security-posture layer** — `tests/security-posture/test_security_posture.py`
   asserts _per-fix_ security contracts against the manifests (offline) and against the
   running cluster (`--live`) — proves each fixed finding **stays fixed**.
3. **Telemetry / observability layer** — `tests/telemetry/test_dashboards.py`
...
4. **Ingress-accessibility layer** — `tests/ingress-accessibility/test_ingress_accessibility.py`
   renders the whole Flux tree and asserts _fleet-wide_ ingress invariants ("what is
...
task test:telemetry       # Telemetry only                (pytest -m telemetry)
...
> `pytest -m <suite>` runs that suite wherever its tests live. `pytest.ini` is the authority on
> which roots are collected and which markers exist — it spans the central `tests/` root plus
> `infrastructure/base` and `applications` so the co-located DR and integration suites are picked
> up without being registered anywhere. Shared utils/fixtures live in `tests/conftest.py` and
> `tests/lib/helpers.py` (plus `tests/lib/dr.py` for the DR machinery); a suite-grouped terminal
> reporter prints a per-suite pass/fail dashboard. `--live` opts into read-only running-cluster
> probes; `--destructive` arms DR chaos. `task test` runs everything offline. **Jest has been
> fully removed** — there is no root `package.json` and no `npm test` path any more; the
> traefik/vpn DR suites and the discord-webhook, mail-relay and crossplane-demo provisioning
> integration suites are all pytest.
...
Telemetry is the fourth suite and is listed above; it is omitted from this table because its
axes are panel-level rather than corpus-level and forcing it into these rows obscures both.
...
| Location     | `infrastructure/base/<component>/tests/`, `tests/etcd-dr/`        | `tests/security-posture/test_security_posture.py`         | `tests/ingress-accessibility/test_ingress_accessibility.py` (+ `ingress_corpus.py`, `ingress_allowlists.py`) |
...
| CI           | (local / on-demand)                                               | `.github/workflows/security-posture.yaml` job `contracts` | same workflow, job `ingress-surface`                                                                         |
| Needs        | `kubectl`                                                         | PyYAML; `kubectl`+LAN for `--live`                        | PyYAML + **`kustomize`**; `kubectl`+LAN for `--live`                                                         |

All of these layers **never mutate real data or black-hole a real route by accident** — chaos
only ever targets throwaway routes and self-healing infra pods; the live security and accessibility
...
enumerates the _whole surface_ so a NEW route that reintroduces a finding's shape (an un-gated `/api`
...
Each deployable component with a failure mode carries a co-located DR suite proving its recovery
story. Suites carry `pytestmark = pytest.mark.disaster_recovery`, so `pytest -m disaster_recovery`
runs them all wherever they live. Shared machinery lives in `tests/lib/dr.py` (kubectl/shell
...
- **Suites:** `cnpg-dr` (Postgres primary failover), `velero-dr` (backup+restore), `etcd-dr`
  ...
  The suite name tracks the _subject_, not the directory — `cnpg-dr` lives under
  `infrastructure/base/databases/tests/` and `velero-dr` under `infrastructure/base/backup/tests/`.
- **Run:** `task test:dr` (safe) · `task test:dr-armed` (armed) ·
  `pytest -m disaster_recovery <path-to-one-suite>` (one suite).
- **Add a suite:** create `infrastructure/base/<component>/tests/test_<subject>_dr.py`, set
...
> Note: `tests/etcd-dr/` lives at repo root because etcd DR was never a deployable component
> (test-only). The `--destructive` flag and the `destructive` marker are defined in
> `tests/conftest.py`.
...
findings, reproductions, evidence, and the report. The security-posture layer here is the
**standing regression net** that proves each finding stays fixed — offensive tooling finds new
holes; posture tests keep the old ones shut. The 2026-09 ingress engagement is tracked as beads
epic **TALOS-lxz5**, whose verification epic wires a posture test to every fix.
