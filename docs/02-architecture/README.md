...
| [service-mesh.md](service-mesh.md)                       | Service mesh strategy - Cilium eBPF mTLS for hybrid cluster integration. **CAUTION: its "Enabling mTLS" instructions target a feature deprecated in Cilium v1.20 and removed in v1.21** — read [cilium-spire-disposition.md](cilium-spire-disposition.md) first | Understanding service mesh implementation                    |
...
| [cilium-spire-disposition.md](cilium-spire-disposition.md) | **INVESTIGATION / RECOMMENDATION** - Should Cilium's bundled SPIRE stay? Verified: 0 policies use mutual auth, the feature is deprecated in the v1.20.0 we run (removed in v1.21), the datastore is reconstructible from CiliumIdentity, nothing fails closed, and the expired-token agent wedge recurred *after* the durable fix. Recommendation: disable and remove (TALOS-1d4o) | Before touching `cilium-spire`, planning a talos03 reset, or reading [service-mesh.md](service-mesh.md) — which still tells you to *enable* this feature |
