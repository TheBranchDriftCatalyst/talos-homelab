...
> ⚠️ **Level 0 is a historical record, not the current procedure.** It documents how the
> original SINGLE-NODE cluster was built by hand, and the commands in it will not work as
> written: `configs/controlplane.yaml` no longer exists, and `talosctl gen config` would mint
> a secrets bundle unrelated to `configs/talsecret.yaml`, producing configs with a CA no node
> in this cluster trusts.
>
> Machine configs are now generated by talhelper from `configs/talconfig.yaml`, and the
> cluster is five nodes rather than one. For the current flow:
>
> | Task | What it does |
> | --- | --- |
> | `task talos:provision` | Preflight + print the ordered rebuild command sequence. Applies nothing. |
> | `task talos:gen-config` | Regenerate `configs/clusterconfig/` from `talconfig.yaml` |
> | `task talos:verify-dry-run` | Ask each node what applying the generated config would do (read-only) |
> | `task talos:apply-config NODE=<host>` | Apply one node's generated config (destructive) |
>
> Kept as-is because it records what was actually done and why, including the hostname-change
> cleanup — which is still worth reading before anyone renames a node.
