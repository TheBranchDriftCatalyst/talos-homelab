...
# State files (support env var override for container deployment)
STATE_FILE="${STATE_FILE:-$REPO_ROOT/.output/worker-state.json}"
SSH_KEY="${SSH_KEY:-$REPO_ROOT/.output/ssh/$KEY_NAME.pem}"
LIGHTHOUSE_STATE="${LIGHTHOUSE_STATE:-$REPO_ROOT/.output/lighthouse-state.json}"
